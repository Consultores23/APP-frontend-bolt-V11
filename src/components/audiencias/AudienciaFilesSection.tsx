// src/components/audiencias/AudienciaFilesSection.tsx
...
interface AudienciaFilesSectionProps {
  audienciaId: string;
  processId: string;
}

const AudienciaFilesSection: React.FC<AudienciaFilesSectionProps> = ({
  audienciaId,
  processId,
}) => {
  ...
  // Fetch audiencia's attached files
  const fetchAudienciaFiles = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error fetching audiencia files:', err.message);
      toast.error('Error al cargar los archivos de la audiencia.');
    }
  }, [audienciaId]);
  ...
  useEffect(() => {
    fetchBucketName();
    fetchAudienciaFiles();
  }, [fetchBucketName, fetchAudienciaFiles]);
  ...
  if (files.length === 0) {
    return (
      <div className="text-center text-gray-400 py-8">
        ...
        <p>No hay archivos adjuntos en esta audiencia.</p>
        <p className="text-sm mt-2">Los archivos aparecerán aquí cuando se adjunten a la audiencia.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="border-b border-dark-600 pb-3">
        ...
        <p className="text-sm text-gray-400 mt-1">
          Archivos relacionados con esta audiencia
        </p>
      </div>
...
export default AudienciaFilesSection;