// src/components/actividades/ComentariosSection.tsx
...
const ComentariosSection: React.FC<ComentariosSectionProps> = ({ activityId, responsables }) => {
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('comentariosactividades') // Corrected table name
        .select('*')
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      ...
      setComments((prev) => [...prev, data]); // Add new comment to the end for chat-like display
      setNewCommentText('');
  ...
  return (
    <div className="space-y-6 flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2"> {/* Added custom-scrollbar and pr-2 for scrollbar space */}
        {isLoading ? (
          ...
          <div className="space-y-4">
            {/* Reversed order for chat-like display */}
      ...
      {/* New Comment Section */}
      <div className="bg-dark-700 rounded-lg p-4 border border-dark-600 mt-6 flex-shrink-0"> {/* Added flex-shrink-0 */}
