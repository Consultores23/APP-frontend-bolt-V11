// src/components/actividades/FilesSelection.tsx
...
interface FilesSelectionProps {
  ...
  initialSelectedFiles?: string[]; // Archivos inicialmente seleccionados
}

const FilesSelection: React.FC<FilesSelectionProps> = ({
  processId,
  onSelectionChange,
    ...
    const fetchBucketName = async () => {
      ...
      try {
        ...
        if (data?.bucket_path) {
          ...
          toast.error('No se encontró el bucket para este proceso.');
        }
      } catch (err: any) {
  ...
  const fetchAllFiles = useCallback(async () => {
    ...
    try {
      ...
      const processedFiles: ProcessedFile[] = cloudFiles
        .filter(file => file.name !== '' && !file.name.endsWith('/')) // Filter out empty names and directories
        .map(file => ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs text-gray-500 pt-3 border-t border-dark-600">
        <p>Los archivos seleccionados se adjuntarán a esta actividad y podrán ser consultados posteriormente.</p>
      </div>
    </div>
  );
};
